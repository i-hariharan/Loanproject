...
//import "../aboutus/MainContent.css"
  ...
  const features = [
    {
      ...
      img: img,
      description:
        "Easy Process assesses a borrower's ability to repay a loan,eligible borrowers with a guaranteed loan amount and interest rate, considering factors like income, credit score, and financial history.",
  ...
  const feedback = [
    {
      id: 1,
      img: feedbackimg,
      title: "Description about customer thoughts",
    },
    {
      id: 2,
      img: feedbackimgg,
  ...
  return (
    <div className="min-h-screen w-full mx-auto">
        ...
        <div className="grid max-md:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 lg:space-x-10 max-md:space-y-5">
          {features.map((features) => (
            ...
            >
              ...
              <p className="text-center mx-auto text-justify mt-5">
        ...
        <div className="grid lg:grid-cols-3 max-md:grid-cols-1 justify-center gap-5">
          {feedback.map((feedback) => (
            ...
            >
              ...
              <p className="flex justify-center text-justify text-center mx-auto text-black">
                {feedback.title}
              </p>
            </div>
          ))}
      ...
      <div className="mx-auto px-4 py-10 w-full bg-blue-900">
        ...
        <div className="m-5 text-center py-[20px] rounded-lg">
          <div className="w-[95%] mx-auto space-x-2">
            <Slider {...settings}>
              {trustdata.map((data) => (
                <div
                  key={data.id}
                  className=" rounded-md p-2 bg-white w-[100px] "
                >
                  <img
                    src={data.img}
                    alt=""
                    className="mx-auto h-[150px] w-[300px] rounded-lg"
                  />
                  <p className="text-lg font-bold mt-3">{data.title}</p>
                  <p className="text-md font-normal mt-5 text-justify text-center mx-auto lg:w-[300px] max-md:w-[250px]">
                    {data.desc}
                  </p>
                </div>
              ))}
